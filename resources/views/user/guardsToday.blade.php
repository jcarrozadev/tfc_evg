...
@section('content')
    <div class="container py-5">
        <h1 class="text-center mb-4 fw-bold fs-2">Guardias del día - {{ \Carbon\Carbon::now()->format('d/m/Y') }}</h1>
                    ...
                    <div class="col-md-12">
                        <div class="card shadow-sm">
                            ...
                            <div class="card-body">
                                ...
                                <p class="mb-1"><strong>Texto guardia:</strong> {{ $guard['text_guard'] ?? '—' }}</p>
                                <p class="mb-1"><strong>Profesor Sustituto:</strong> {{ $guard['covering_teacher_name'] ?? 'Sin asignar' }}</p>
                            </div>
                            <div class="card-footer text-muted text-end">
                                Guardia actualizada a las {{ \Carbon\Carbon::parse($guard['created_at'])->format('H:i') }}
                            </div>
                        </div>
                    </div>
                @endforeach
